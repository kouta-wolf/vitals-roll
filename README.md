...
TRPGのセッション中に発生するバフ管理の手間とミスを解消するアプリ。バフの持続時間を自動でカウントし、バフ反映済みの判定値をクリップボードにコピーすることで、状態管理をアプリに任せてゲームへの集中を維持できるアプリ。
...
訪問看護の現場では、患者ごとのバイタルサイン（体温、脈拍、血圧など）の変化や、状態の経過、治療内容、薬効を常に把握することが求められる。「いつ、何が、どう変わったか」の記録が抜けると、次の判断を誤るリスクに直結する。状態の経過を正確に管理することの重要性を、仕事を通じて理解していた。

TRPGでのミスに既視感を覚えたのも、看護師の仕事と同じ構造に見えたためだ。キャラクターのHPやMP、ステータス管理や魔法の持続効果は、バイタルサインや状態経過と同じような考え方であり、それがズレるとセッションの質にも直結する。

看護の現場では記録の抜けを防ぐ仕組みがある程度整備されているが、TRPGでは手動管理に頼りきりになっているという差が気になる。
...
TRPGでは戦闘中にさまざまな魔法や技能が使用されるが、あの魔法は何ターン有効かという情報は自動で記録されないため、自分でメモを取る必要がある。しかしゲームに集中するほどメモを取り忘れ、取れたとしても管理が追いつかず、気づいたときには効果が切れているはずの数値を使い続けていたというミスが起きる。
...
「バフ管理に悩むTRPGプレイヤーが、状態管理をアプリに任せてゲームに集中できるようにするサービス」
...
ゲームのたびに使うツールのため習慣化しやすい。バフデータはセッションを重ねるごとに充実するため使えば使うほど毎回の準備が簡単になる構造である。複数人対応ができれば全員でバフ管理ができるためゲームの質も向上する。
