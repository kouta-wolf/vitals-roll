...
また、セッション後に経緯を振り返ろうとすると、記録には「HPが30になった」という結果しか残っておらず、そこに至った経緯として攻撃を受けたのか回復したのかが追えなくなっていたこともあった。
...
看護師として働いていた経験が影響している。

訪問看護の現場では、患者ごとのバイタルサイン（体温、脈拍、血圧など）の変化や、状態の経過、治療内容、薬効を常に把握することが求められる。「いつ、何が、どう変わったか」の記録が抜けると、次の判断を誤るリスクに直結する。状態の経過を正確に管理することの重要性を、仕事を通じて体で覚えていた。

TRPGでのミスに違和感を覚えたのも、看護師の仕事と同じ構造に見えたため。キャラクターのHPやMP、ステータス管理や魔法の持続効果は、バイタルサインや状態経過と同じような考え方であり、それがズレるとセッションの質にも直結する。

看護の現場では記録の抜けを防ぐ仕組みがある程度整備されているのに、ゲームの場では手動管理に頼りきりになっているという差が気になる。
...
魔法によるステータス強化の効果時間を手動で管理しなければならない点。

TRPGのセッションでは戦闘中にさまざまな魔法や技能が使用されるが、あの魔法は何ターン有効かという情報は自動で記録されないため、自分でメモを取る必要がある。しかしゲームに集中するほどメモを取り忘れ、取れたとしても管理が追いつかず、気づいたときには効果が切れているはずの数値を使い続けていたというミスが起きる。

また、HPやMPの増減も、ツールのログには結果の数値しか残らない。なぜその数値に変化したのかという経緯が追えないため、後から見返しても状況を把握できない。
...
ゆとシートIIはキャラクター作成ツールとして完成度が高く私も愛用している。しかしセッション中に閲覧やメモをするツールであって、動的に管理するツールとしては設計されていない。バフの残りターンを管理する機能はなく、ステータスが変化するたびに手動でチャットパレットを書き換える必要がある。
